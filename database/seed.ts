import { PrismaClient, FollowUpStatus, Gender, InsuranceStatus, PetSpecies, PolicyType, UserRole, VaccineStatus, VisitType } from '@prisma/client';
...
async function main() {
  ...
  const record = await prisma.medicalRecord.create({
    data: {
      ...
      cost: 328,
      nextVisitDate: new Date(Date.now() + 1000 * 60 * 60 * 24 * 5),
      attachments: [],
    },
  });
